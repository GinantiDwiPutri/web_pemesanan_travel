...
function table() {
    $.get("registrants/table", function (data) {
...
$(document).ready(function () {
    ...
    $("body").on("click", ".accept", function () {
        if (confirm("Yakin ingin menerima driver ini menjadi bagian dari Driver Berangkut?")) {
            var id = $(this).data("id");
            $.ajax({
                url: "registrants/accept",
                method: "POST",
                data: { id: id },
                success: function (response) {
                    if (response.status == 201) {
